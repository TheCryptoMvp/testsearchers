...
async function getAllPairs(factoryContract, length = 1000) {
    const pairPromises = [];
...
async function fetchPairs() {
    let allPancakePairs = await getAllPairs(pancakeSwapV2FactoryContract);
    let allSushiPairs = await getAllPairs(sushiSwapFactoryContract);

    // Filter pairs with sufficient liquidity
    pancakePairs = await filterPairsWithLiquidity(allPancakePairs, pancakeSwapV2FactoryContract);
    sushiPairs = await filterPairsWithLiquidity(allSushiPairs, sushiSwapFactoryContract);
}
...
async function main() {
    while (true) {
        try {
            await fetchPairs(); // Fetch the latest pair data
            pancakePrices = await getPrices(pancakePairs, pancakeSwapV2FactoryContract);
            sushiPrices = await getPrices(sushiPairs, sushiSwapFactoryContract, true);
